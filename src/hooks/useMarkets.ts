...
export function useMarkets(): UseMarketsReturn {
  ...
  const fetchDecentralizedMarkets = async (): Promise<Market[]> => {
    ...
    try {
      ...
      console.log(
        `Recherche des événements du bloc ${startBlock} au bloc ${currentBlock}`
      )
      ...
      const marketsPromises = logs.map(async (log) => {
        try {
          ...
          try {
            ...
            market.engagementDeadline = new Date(
              Number(engagementDeadline) * 1000
            )
            market.resolutionDeadline = new Date(
              Number(resolutionDeadline) * 1000
            )
            market.creatorFee = (creatorFee as bigint).toString()
            market.predictionCount = Number(predictionCount)
            market.ipfsMetadataHash = ipfsMetadataHash as string

            if (market.ipfsMetadataHash) {
              try {
                const ipfsGatewayUrl = `https://ipfs.io/ipfs/${market.ipfsMetadataHash.replace(
                  "ipfs://",
                  ""
                )}`
                ...
                market.title = metadata.name || market.title
                market.description = metadata.description
                market.image = metadata.image
                  ? `https://ipfs.io/ipfs/${metadata.image.replace(
                      "ipfs://",
                      ""
                    )}`
                  : undefined
                market.events = metadata.properties?.events || []
              } catch (ipfsError) {
                console.warn(
                  `Impossible de récupérer les métadonnées IPFS pour ${market.ipfsMetadataHash}:`,
                  ipfsError
                )
              }
            }
            ...
            console.warn(
              `Impossible de récupérer les détails pour le marché ${market.instanceAddress}:`,
              detailError
            )
          }
      ...
      const validMarkets = marketsResults.filter(
      ...
      console.error(
        "Erreur lors de la récupération des marchés décentralisés:",
        error
      )
      throw error
    }
